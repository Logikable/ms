...
struct CombatType {
  ...
  // How one of these wanders its arena, from the spawn that named it. Unset
  // for everything that stands still. See Spawn.
  ArenaWalk walk;
...
struct AttackOption {
  ...
  // Share of the pool a landed strike of THIS attack puts back, on top of
  // whatever the character's passives recover on any swing. Angel Ray heals
  // the Bishop as it lands, and Darkness Aura's pulse heals every time it
  // fires. Costs no swing, unlike heal_fraction above -- the damage still
  // goes out.
  double hp_recover_pct = 0.0;
