...
// The columns inside a window's border, and how a row divides them. Fixed:
// the three windows stand under the one header, so a column that grew with
// what was put in it would leave the header naming the wrong one.
...
// What one effect of a skill reads as in a column: "Crit Rate +9%". The
// separator is the item list's, a cell there listing several effects too.
...
void LinkSkillPanel::Reset() {
  ...
  // The preset in play: the farming slot with the autoswap on, the one put in
  // use with it off. Where the player left the bar is not kept -- a screen
  // opens on what the character is carrying.
  preset_ = character_.SlotFor(PresetKind::kLinkSkills, Activity::kFarming);
  enabled_row_ = 0;
  all_row_ = 0;
  // On the bar rather than in the list: an empty preset has nothing to stand
  // on, and the bar is what the player came to the middle window for.
...
std::vector<const Skill*> LinkSkillPanel::AllSkills() const {
  ...
  for (const Skill* skill :
       SkillsForAdvancement(skills_, JOB_ADVANCEMENT_LINK)) {
    // A line nobody on the account has taken past its first rung pays
    // nothing, and a row reading 0 would only advertise a skill that is not
    // on offer.
...
int LinkSkillPanel::RowHere() const {
  switch (zone_) {
    // The top window holds one row, so its cursor never moves.
    case LinkZone::kMine:
      return 0;
    case LinkZone::kEnabled:
      return enabled_row_;
...
int LinkSkillPanel::StopsHere() const {
  ...
  // The preset bar is the stop above the middle window's rows, and the one
...
void LinkSkillPanel::MoveRow(int delta) {
  ...
  if (zone_ == LinkZone::kMine) {
    return;  // One row, and nothing above or below it inside the window.
  }
  if (zone_ == LinkZone::kAll) {
    all_row_ = StepCursor(ClampedRow(rows), delta, stops);
    return;
  }
  // Stop 0 is the preset bar and the rows are the stops after it, so Up off
  // the first row lands on the bar and Down off the last comes back to it.
...
void LinkSkillPanel::RemoveSelected() {
  ...
  // The row that slid up into this place is the next skill; when the preset
  // has run out there is nothing to stand on and the cursor climbs to the bar.
...
void LinkSkillPanel::OpenMenu() {
  ...
  if (at.kind == LinkCursor::Kind::kPreset) {
    preset_menu_.Reset();
    // Nothing to put in use while the autoswap is picking, and nothing to do
    // to the one already in use. The entry stays visible either way, exactly
    // as the Hyper tab's does.
...
LinkMenuChoice LinkSkillPanel::menu_choice() const {
  ...
  // The entry between Inspect and Close, which the top window does not have:
  // a skill held for free is neither added nor taken off.
...
ftxui::Element LinkSkillPanel::RenderEnabled() const {
  ...
  if (focused) {
    // The preset rides in the key, so the same row of another preset counts
    // as a different skill and starts from its own head.
  ...
  for (int i = 0; i < kMaxEquippedLinkSkills; ++i) {
    if (i >= static_cast<int>(held.size())) {
      // The slots a preset has not filled, drawn so the window is one size
      // whatever it holds -- and so an empty preset says so on its first row.
...
int LinkSkillPanel::MenuRow() const {
  ...
  // menu floats from the screen's own corner -- so the screen's top comes off.
  // One row back from the cursor, so the highlighted entry lands beside what
  // the menu is about rather than below it.
