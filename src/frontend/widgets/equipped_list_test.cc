...
class EquippedListTest : public PanelTest {
 protected:
  // The equipped list's columns at `width`, with every feature unlocked.
...
// A staff has both attack stats. The row shows the one the wearer's damage
// uses, not whichever the item lists first.
TEST_F(EquippedListTest, ShowsTheAttackTheJobSwingsWith) {
  ...
  // Wide enough for the stats column, which a narrow panel drops first.
...
// Arrows without a bow are worn but do nothing, so the list shows them dimmed.
// The row still says what the item is.
...
// Callers split the row at the end of the name cell to colour the name
// separately from the columns after it. A name too long for its column is cut,
...
// The name column grows with the panel, and the header above it grows with the
// rows.
TEST_F(EquippedListTest, AWideNameColumnHoldsTheWholeName) {
  ...
  // Narrow enough that the stats column is shown and the name is at its
  // shortest, compared with a panel wide enough to fill the potential column
  // and still fit the whole name.
  ...
  EXPECT_NE(wide[0].text.text.find("Metallic Blue Book (Antistrophe)"),
            std::string::npos);
  // The header shifts with them, so the columns are still labelled correctly.
...
TEST_F(EquippedListTest, ListsWhatIsWornInTheWindowsOrder) {
  ...
  proto.set_job_stage(2);
  CharacterInstance c(rng_, std::move(proto));
  // Worn in reverse of the expected order, so the output order comes from the
  // list, not the order they were equipped.
...
// at that width it still shows the name, the slot and all three upgrades, which
