...
textprotos are: the file a player runs is the whole game. The tracks go in as
whole MP3s, and miniaudio decodes them at play time.

They're included with assembler `.incbin` directives rather than a C array,
because over 100 MB of audio would be over 100 million tokens for a compiler to
parse, and is no work for an assembler. The generated `.cc` is only the table
naming them.
"""
...
def _frame(data, pos):
    ...
    None means `pos` isn't a frame header, which is how the walk below skips
    past a tag or padding.
    """
    if pos + 4 > len(data) or data[pos] != 0xFF or data[pos + 1] & 0xE0 != 0xE0:
        return None
    version = _VERSIONS.get(data[pos + 1] >> 3 & 3)
    # Layer III only, which is what every track under bgm/ uses.
...
def duration_ms(path):
    ...
    Exact for variable and constant bitrates alike, and decodes nothing: each
    header says how far away the next one is. Returns 0 for a file with no
    frames.
    """
...
def write_asm(out, paths, include_dir):
    ...
    A .S (capital S) is preprocessed, which lets one file name the read-only
    section correctly for both ELF and the Windows cross-build.
    """
