...
std::vector<std::string> WrapBalanced(const std::string& text, int width,
  ...
  // best[i] is the cost of laying out the words from i onward: the number of
  // lines, then the longest line. Fewest lines wins first, and balance is
  // decided among layouts with the same number of lines.
  int count = static_cast<int>(words.size());
  std::vector<std::pair<int, int>> best(count + 1, {0, 0});
  std::vector<int> next(count + 1, count);
  for (int i = count - 1; i >= 0; --i) {
    best[i] = {count + 1, 0};
    // Every line except the first has the indent, and a line starting at any
    // word but the first is never the first line.
    int line = i > 0 ? indent : 0;
    for (int j = i; j < count; ++j) {
      line += static_cast<int>(words[j].size()) + (j > i ? 1 : 0);
      // The last line is the one that shares its row with the value.
...
std::string DropChance(double per_kill) {
  ...
  // Trailing zeros carry no information here, and a bare decimal point even
  // less.
...
std::string FormatCompact(int64_t n) {
  ...
  for (const Unit& unit : kUnits) {
    ...
    // Three significant digits wherever the decimal point falls, with no
    // trailing zeros: a column of numbers is read for size, not the last
    // decimal.
